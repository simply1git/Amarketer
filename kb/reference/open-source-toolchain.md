...
- **aider** (Apache 2.0) — powers the **cloud autonomous brain** on the owner's chosen **OpenRouter** free-tier key (`.github/workflows/agent-task.yml`, `OPENROUTER_API_KEY` secret, optional `OPENROUTER_MODEL` variable; runs with the PC off).
- **gemini-cli** (Google, Apache 2.0) — documented cloud-brain fallback (workflow auto-uses it if only `GEMINI_API_KEY` is set); `.gemini/settings.json` points its context file at CLAUDE.md.
